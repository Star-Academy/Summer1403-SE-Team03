...
public class SearchResultsFilter
{
    ...
    private IEnumerable<string> GetIntersection(IEnumerable<string> firstCollection, IEnumerable<string> secondCollection)
    {
        return firstCollection.Intersect(secondCollection).ToList();
    }
    
    private IEnumerable<string> RemoveExcludedElements(IEnumerable<string> source, IEnumerable<string> elementsToExclude)
    {
